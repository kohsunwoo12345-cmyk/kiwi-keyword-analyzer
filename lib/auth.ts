...
export interface ModelPriceRow {
  ...
  /** 자체 기능(업스케일)의 서비스 요금 기준가(원/단위). 제공사 원가가 없는 항목만 값이 있다 */
  selfFee?: number | null
  selfFeeUnit?: string | null
}
...
export interface AcademyRow { name: string; members: number; price: number | null }
export async function adminUserMarkups(q = ''): Promise<{ ok: boolean; error?: string; users?: UserMarkupRow[]; defaultMarkup?: { video: number; image: number }; refSurchargeDefault?: number; cnSurchargeDefault?: number; creditPriceDefault?: number; promptgenCredits?: number; promptgenMarkup?: number; academyPrices?: Record<string, number>; academies?: AcademyRow[] }> {
  try {
...
export interface NoticeHistoryItem {
  ...
  receivedAt: string; readAt: string | null; read: boolean
}
export async function fetchNoticeHistory(): Promise<{ ok: boolean; history: NoticeHistoryItem[] }> {
  try {
    const r = await fetch('/api/notices?history=1', { credentials: 'include', cache: 'no-store' })
    const d = await r.json()
    return { ok: !!d.ok, history: d.history || [] }
  } catch { return { ok: false, history: [] } }
}
...
export interface CrmPreview { all: number; active: number; byPlan: { plan: string; n: number }[]; emailAll?: number; emailActive?: number }
...
export async function crmSend(p: {
  segment: 'all' | 'plan' | 'active'; plan?: string; channel: 'sms' | 'alimtalk' | 'email'; content: string; sender?: string
  subject?: string; scheduleAt?: string; tplCode?: string; test?: boolean; testPhone?: string; testEmail?: string
...
/** 관리자 직접 이메일 발송(Resend) — 직접 수신자(to) 또는 세그먼트 */
export async function adminSendEmail(p: {
  to?: string; segment?: 'all' | 'plan' | 'active'; plan?: string; subject: string; content: string
