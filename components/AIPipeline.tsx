...
const STAGES: Stage[] = [
  {
    ...
    accent: '#38bdf8',
    body: (
      <div className="rounded-lg bg-black/40 p-2 font-mono text-[10px] leading-relaxed text-sky-200/90">
        노을 지는 해변, 걷는 인물
        <br />
        cinematic · 9:16 · 5s
      </div>
  ...
  return (
    <section className="relative overflow-hidden border-t border-white/10 py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-5">
        ...
        {/* 파이프라인 (하나씩 등장) */}
        <div className="mt-16 flex flex-col items-stretch gap-4 lg:flex-row lg:items-stretch">
          {STAGES.map((s, i) => {
            const Icon = s.icon
            return (
              <div key={s.key} className="flex flex-1 flex-col items-center lg:flex-row">
                <Reveal variant="rise" delay={i * 220} className="w-full">
                  <div className="card hover-lift relative h-full overflow-hidden p-4">
                    <div
                    ...
                    <div className="relative flex items-center gap-2">
                      <span
                        className="grid h-9 w-9 flex-shrink-0 place-items-center rounded-xl text-white shadow-lg"
                        style={{ background: `linear-gradient(135deg, ${s.accent}, #2563eb)` }}
                      >
                        <Icon size={17} />
                      </span>
                    ...
                    <div className="relative mt-3">{s.body}</div>
                  </div>
                </Reveal>

                {/* 연결 화살표 */}
                {i < STAGES.length - 1 && (
                  <Reveal delay={i * 220 + 130} className="flex items-center justify-center py-1 lg:px-1 lg:py-0">
                    <span className="grid h-7 w-7 place-items-center rounded-full border border-blue-400/30 bg-blue-500/10 text-blue-300">
        ...
        <Reveal delay={200} className="mt-14">
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {MODELS_VIDEO.map((m, i) => (
                <Reveal key={m} delay={i * 55} variant="scale">
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-white/12 bg-white/[0.05] px-3 py-1.5 text-xs font-medium text-slate-200">
