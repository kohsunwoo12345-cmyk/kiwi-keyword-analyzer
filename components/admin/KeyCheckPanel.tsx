...
/* 관리자 — 제공사 API 키 확인 (LTX·Recraft 공용)
   이 화면이 하는 일은 딱 하나다: **콘솔에 넣어 둔 키가 진짜 되는가.**

   ⚠ 여기서는 아무것도 만들어지지 않는다. 서버 진단이 GET(읽기)만 보내기 때문이다.
   ...
   ⚠ 화면은 제공사마다 따로 만들지 않는다. 서버 판정이 한 군데(_keycheck.ts)에 있는데
     화면만 두 벌이면, 판정이 늘어날 때 한쪽에만 반영되는 날이 온다. */
...
/* 상태코드 하나가 무슨 뜻인지 — 이 표가 이 화면의 절반이다.
...
function 뜻(status: number): { label: string; tone: 'ok' | 'bad' | 'mid' | 'dead' } {
  ...
  if (status === 405) return { label: '메서드 다름 — 주소는 있음', tone: 'mid' }
...
const fmtBalance = (v: number, unit?: string) =>
  unit === 'USD'
    ? '$' + v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    : v.toLocaleString('ko-KR') + (unit ? ' ' + unit : '')

export function KeyCheckPanel({
  provider, title, envName, desc, registryHref,
}: {
  /** 서버 진단 이름 — /api/generate?diag=<provider> */
  provider: string
  title: string
  /** 화면에 보여 줄 환경변수 이름 */
  envName: string
  ...
  const [res, setRes] = useState<KeyCheckRes | null>(null)
  const [loading, setLoading] = useState(false)

  const run = useCallback(async () => {
    setLoading(true)
    try {
      const r = await fetch(`/api/generate?diag=${encodeURIComponent(provider)}`, { credentials: 'include', cache: 'no-store' })
      setRes(await r.json())
    } catch (e) {
      setRes({ error: String((e as Error)?.message || e) })
    }
    setLoading(false)
  }, [provider])
  useEffect(() => { run() }, [run])
  ...
  //  대조에 쓴 주소에 우리 키로 무슨 답이 왔는지 — 두 줄을 나란히 놓아야 판정이 읽힌다
  const 우리응답 = res?.결과?.find((x) => x.url === res?.대조?.url)
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
        ...
        <span>
          <b>생성은 하지 않습니다.</b> 확인에 쓰는 요청은 전부 읽기(GET)이고 본문이 없습니다 —
          만들어질 결과물이 없으니 이 화면을 몇 번 눌러도 요금이 나가지 않습니다.
        </span>
      </div>
      ...
      {loading && !res && (
        <Panel><p className="flex items-center gap-2 text-sm text-[var(--text-dim)]"><Loader2 size={14} className="animate-spin" /> 제공사에 물어보는 중…</p></Panel>
      )}
      ...
      {res && !res.needAdmin && (
        <>
          ...
          <Panel className="mb-4">
            <div className={cn('flex items-start gap-3 rounded-xl border px-4 py-3', TONE[verdictTone])}>
              ...
              <div className="min-w-0">
                <p className="text-base font-bold">{res.error && !res.판정 ? res.error : res.판정 || '판정 없음'}</p>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-lg border border-[var(--border)] p-3">
                ...
                <p className="mt-0.5 font-semibold">
                  {res.잔액 == null ? '조회 안 됨' : fmtBalance(res.잔액, res.잔액단위)}
                </p>
              </div>
            </div>

            {res.다음 && <p className="mt-3 text-xs text-[var(--text-dim)]">다음 단계 — {res.다음}</p>}
            {res.키작동 === false && res.콘솔 && (
              <p className="mt-2 text-xs">
                <a href={res.콘솔} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-semibold text-violet-600 hover:underline">
                  키 재발급 콘솔 열기 <ExternalLink size={11} />
                </a>
              </p>
            )}
          </Panel>
                ...
                <table className="w-full min-w-[520px] text-sm">
                  <tbody>
                    <tr className="border-b border-[var(--border)]">
                      ...
                      <td className="py-2 font-mono">{우리응답?.status ?? '—'}</td>
                      <td className="py-2 text-[var(--text-dim)]">{뜻(우리응답?.status ?? 0).label}</td>
                    </tr>
                    <tr>
          ...
          {!!res.모델목록?.length && (
            <Panel className="mb-4" title={<span className="flex items-center gap-2"><Boxes size={16} className="text-violet-500" /> 이 키로 잡히는 모델</span>}>
              ...
              <p className="mt-3 text-xs text-[var(--text-dim)]">
                ...
                <a href={registryHref} className="ml-1 inline-flex items-center gap-1 font-semibold text-violet-600 hover:underline">
          ...
          {/* 주소를 어떻게 정했는지 숨기지 않는다 — 숨기면 다음 사람이 같은 벽을 다시 만난다 */}
          {res.주소근거 && (
            <p className="mt-4 text-xs leading-relaxed text-[var(--text-dim)]">확인 주소 — {res.주소근거}</p>
          )}
        </>
      )}
    </div>
