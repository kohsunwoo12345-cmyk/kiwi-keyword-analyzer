...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  ...
  const tools = {
    ...
    '문자·알림톡(알리고)': { ok: has('ALIGO_API_KEY') && has('ALIGO_USER_ID'), keys: { ALIGO_API_KEY: has('ALIGO_API_KEY'), ALIGO_USER_ID: has('ALIGO_USER_ID'), ALIGO_SENDER_optional: has('ALIGO_SENDER'), ALIGO_SENDER_KEY: has('ALIGO_SENDER_KEY'), ALIGO_PROXY_URL: has('ALIGO_PROXY_URL') } },
