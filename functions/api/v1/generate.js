...
import { getUserByApiKey, logApiCall, hasVideoApiAccess, ensureApiKeysSchema } from "../_apikeys";
...
export const onRequestPost = async ({ request, env }) => {
  ...
  try {
    ...
    if (me.role !== "admin" && (Number(me.credits) || 0) < (est?.credits || 0)) {
  ...
  if (ok) {
    ...
    await logApiCall(db, { keyId, userId: me.id, endpoint: "/api/v1/generate", provider, model, kind, credits: charged, status: "ok" });
    return json({ ok: true, ...data, credits_charged: charged, credits_remaining: Number(me.credits) || 0 });
  }
  await logApiCall(db, { keyId, userId: me.id, endpoint: "/api/v1/generate", provider, model, kind, credits: 0, status: "failed", error: data.error || `HTTP ${res.status}` });
