// POST /api/team/upload  (multipart: teamId, file) — 팀 채팅 사진/영상 업로드
//  · R2 저장(미설정 시 D1 media_blobs 폴백). 반환 key 는 /api/media/<key> 로 서빙.
...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  ...
  if (!teamId || !(await isMember(db, teamId, me.id))) return json({ ok: false, error: '팀 멤버가 아닙니다.' }, 403)
  ...
  const key = `team-chat/${teamId}/${Date.now()}_${crypto.randomUUID().slice(0, 8)}.${ext}`
  ...
  if (R2) {
    ...
    // D1 폴백 — media_blobs 에 저장(서빙 라우트가 폴백 지원)
