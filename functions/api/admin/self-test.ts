...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const defs: { id: string; name: string; covers: string; keys: string[]; run?: (key: string) => Promise<{ ok: boolean; status: number; msg: string }> }[] = [
    ...
    { id: 'seedream', name: 'Seedream (ByteDance)', covers: 'Seedream 5.0 Pro/Lite · 4.5 · 4.0 · 3.0 · SeedEdit (이미지) · diag=seedream-all 로 실검증', keys: ['Seedance_API_KEY', 'SEEDANCE_API_KEY', 'seedance_api_key'] },
