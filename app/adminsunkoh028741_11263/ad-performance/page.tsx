  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="space-y-6 p-6 lg:p-8">
        {/* URL 분석기 */}
        <Reveal>
          <Panel title={<span className="flex items-center gap-2"><Search size={16} className="text-blue-500" /> 랜딩페이지 URL 분석</span>}>
            <div className="flex flex-wrap gap-2">
              ...
              />
              <Button size="sm" onClick={() => analyze()} disabled={analyzing || !urlInput.trim()}>
                ...
                {!detail.found ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <a href={detail.landing.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-md bg-blue-50 px-2 py-0.5 font-mono text-[11px] text-blue-600 hover:underline">{detail.landing.url} <ExternalLink size={11} /></a>
                    </div>
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      <div className="rounded-xl border border-[var(--border)] p-3 text-center"><div className="text-[11px] font-semibold text-[var(--text-dim)]">조회수</div><div className="mt-0.5 text-2xl font-bold tabular-nums">{num(detail.landing.views)}</div></div>
                      <div className="rounded-xl border border-[var(--border)] p-3 text-center"><div className="text-[11px] font-semibold text-[var(--text-dim)]">순 방문자</div><div className="mt-0.5 text-2xl font-bold tabular-nums">{num(detail.landing.uniqueVisitors)}</div></div>
                      <div className="rounded-xl border border-[var(--border)] p-3 text-center"><div className="text-[11px] font-semibold text-[var(--text-dim)]">전환(신청)</div><div className="mt-0.5 text-2xl font-bold tabular-nums text-emerald-600">{num(detail.landing.conversions)}</div></div>
                      <div className="rounded-xl border border-[var(--border)] p-3 text-center"><div className="text-[11px] font-semibold text-[var(--text-dim)]">전환율</div><div className="mt-0.5 text-2xl font-bold tabular-nums text-blue-600">{pct(detail.landing.rate)}</div></div>
                    </div>
                          ...
                          <tbody>
                            {(detail.byDay ?? []).map((d) => (
                              <tr key={d.d} className="border-t border-[var(--border)]">
                                ...
                                <td className="px-3 py-2 text-right tabular-nums text-emerald-600">{num(d.conversions)}</td>
                                <td className="px-3 py-2 text-right tabular-nums text-blue-600">{d.views > 0 ? Math.round((d.conversions / d.views) * 1000) / 10 : 0}%</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
        ...
        <Reveal>
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <StatCard label="랜딩페이지" value={num(t?.landings ?? 0)} icon={BarChart3} accent="#2563eb" />
            <StatCard label="랜딩 총 조회수" value={num(t?.landingViews ?? 0)} icon={Eye} accent="#0ea5e9" />
            <StatCard label="랜딩 총 전환" value={num(t?.landingConversions ?? 0)} icon={Target} accent="#22c55e" />
            <StatCard label="광고(알림) 클릭" value={num(t?.adClicks ?? 0)} icon={MousePointerClick} accent="#8b5cf6" />
          </div>
        </Reveal>

        {/* 랜딩 성과 목록 */}
        <Reveal>
          <Panel title={<span className="flex items-center gap-2"><BarChart3 size={16} className="text-blue-500" /> 랜딩페이지 성과 <span className="text-xs font-normal text-[var(--text-dim)]">(조회수·전환율)</span></span>}>
                ...
                <tbody>
                  {landings.length === 0 ? (
                    ...
                    <tr key={l.id} className={cn(TR, 'cursor-pointer')} onClick={() => analyze(l.url)}>
                      ...
                      <td className={cn(TD, 'text-right tabular-nums text-emerald-600')}>{num(l.conversions)}</td>
                      <td className={cn(TD, 'text-right font-semibold tabular-nums text-blue-600')}>{pct(l.rate)}</td>
        ...
        {/* 알림 광고 성과 */}
        <Reveal>
          <Panel title={<span className="flex items-center gap-2"><Megaphone size={16} className="text-blue-500" /> 알림 광고 성과 <span className="text-xs font-normal text-[var(--text-dim)]">(CTA가 우리 랜딩인 알림 = 자동 광고 등록)</span></span>}>
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {ads.length === 0 ? (
                    ...
                    <tr key={a.campaignId} className={TR}>
                      <td className={TD}>
                        <div className="flex items-center gap-1.5 font-semibold"><Bell size={12} className="text-blue-400" /> {a.title}</div>
                        <div className="text-[11px] text-[var(--text-dim)]">{a.target === 'visitors' ? '접속 전체' : a.target === 'all' ? '전체 회원' : a.target === 'plan' ? '요금제별' : a.target}</div>
                      </td>
                      <td className={TD}>
                        {a.isOurLanding
                          ? <a href={a.landingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 font-mono text-[11px] text-blue-600 hover:underline"><Link2 size={11} /> {a.landingUrl}</a>
                      ...
                      <td className={cn(TD, 'text-right tabular-nums text-violet-600')}>{a.clicks == null ? '-' : num(a.clicks)}</td>
                      <td className={cn(TD, 'text-right tabular-nums')}>{pct(a.ctr)}</td>
                      <td className={cn(TD, 'text-right tabular-nums text-emerald-600')}>{num(a.landingConversions)}</td>
                      <td className={cn(TD, 'text-right font-semibold tabular-nums text-blue-600')}>{pct(a.landingRate)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
