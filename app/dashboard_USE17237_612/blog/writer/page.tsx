      ...
      <div className="space-y-6 p-6 lg:p-8">
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          >
            ...
            {note && !loading && (
              <div className="mb-3 rounded-xl border border-amber-500/30 bg-amber-500/12 px-3 py-2 text-xs text-amber-600">
                {note}
              </div>
            )}
            {result && !loading && (
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {result.tags.map((t) => (
                    <Badge key={t} className="border-emerald-500/30 bg-emerald-500/12 text-emerald-600">
                      #{t}
                    </Badge>
                  ))}
                </div>
                ...
                {copied && (
                  <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/15 px-3 py-2.5 text-sm text-emerald-600">
                    본문이 클립보드에 복사되었습니다.
                  </div>
                )}
              </div>
