...
const statusMeta: Record<string, { badge: string; icon: typeof Check }> = {
  승인: { badge: 'border-emerald-500/30 bg-emerald-500/12 text-emerald-700', icon: Check },
  검수중: { badge: 'border-amber-500/30 bg-amber-500/12 text-amber-700', icon: Clock },
  등록됨: { badge: 'border-sky-500/30 bg-sky-500/12 text-sky-700', icon: Clock },
  반려: { badge: 'border-rose-500/30 bg-rose-500/12 text-rose-700', icon: AlertCircle },
}
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="space-y-6 p-6 lg:p-8">
        {msg && (
          <div className={`flex items-center gap-2 rounded-xl border px-4 py-3 text-sm ${msg.ok ? 'border-emerald-500/30 bg-emerald-500/12 text-emerald-700' : 'border-rose-500/30 bg-rose-500/12 text-rose-700'}`}>
            ...
            <div className="space-y-2">
              {channels.map((c) => (
                <div key={c.channelId} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[var(--border-soft)] px-3.5 py-2.5">
                  ...
                  <Badge className="border-emerald-500/30 bg-emerald-500/12 text-emerald-700"><Check size={12} /> 사용중</Badge>
                </div>
              ))}
              <button onClick={() => setRegStep(regStep === 0 ? 1 : 0)} className="text-xs font-medium text-amber-600 hover:underline">+ 채널 추가 등록</button>
            </div>
          ) : (
            <p className="mb-3 rounded-xl border border-amber-500/30 bg-amber-50/50 px-3.5 py-2.5 text-sm text-amber-700">
