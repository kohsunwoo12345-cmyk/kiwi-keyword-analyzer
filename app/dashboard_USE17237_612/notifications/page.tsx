  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        accent={ACCENT}
        action={
          <button onClick={load} className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--border)] px-3 py-1.5 text-sm text-[var(--text-soft)] hover:bg-slate-50">
      ...
      <div className="p-6 lg:p-8">
        <Reveal>
          <Panel title={<span className="flex items-center gap-2"><Bell size={16} className="text-blue-500" /> 받은 알림 <span className="text-xs font-normal text-[var(--text-dim)]">({items.length}건)</span></span>}>
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <span className="grid h-14 w-14 place-items-center rounded-2xl bg-blue-50 text-blue-400"><Bell size={26} /></span>
                    ...
                    <div className="p-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {n.read
                          ...
                          : <span className="inline-flex flex-shrink-0 items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-semibold text-blue-600"><Circle size={12} /> 새 알림</span>}
                      </div>
                      <div className="mt-2 flex items-center gap-3 text-[11px] text-[var(--text-dim)]">
                        ...
                        {n.ctaLabel && n.ctaUrl && (
                          <a href={n.ctaUrl} target={/^https?:\/\//.test(n.ctaUrl) ? '_blank' : undefined} rel="noopener noreferrer"
                            className="ml-auto inline-flex items-center gap-1 font-semibold text-blue-600 hover:underline">
