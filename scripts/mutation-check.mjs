...
const MUTATIONS = [
  ...
  { g: '정직', name: '그룹 목록이 실패해도 다시 "그룹 없음" 으로 답한다',
    file: 'functions/api/funnel/groups.ts',
    from: "    return j({ success: false, error: '그룹 목록을 불러오지 못했습니다.', groups: [] }, 500)",
    to: '    return j({ success: true, groups: [] })' },
  { g: '정직', name: '퍼널 상세가 실패해도 다시 성공으로 답한다 (빈 캔버스가 보인다)',
    file: 'functions/api/funnels/[id].ts',
    from: "    return j({ success: false, error: '요청 처리 중 오류가 발생했습니다.' }, 500)",
    to: '    return j({ success: true, funnel: null, groups: [], connections: [], autoSms: [] })' },
  { g: '정직', name: '자동응답 목록이 실패해도 다시 "자동응답 없음" 으로 답한다',
    file: 'functions/api/funnel/auto-responses.ts',
    from: "    return j({ success: false, error: '자동응답 목록을 불러오지 못했습니다.', rules: [], responses: [] }, 500)",
    to: '    return j({ success: true, rules: [], responses: [] })' },
  { g: '정직', name: '보관함 목록 조회를 다시 삼킨다 ("영상이 없습니다" 가 뜬다)',
    file: 'functions/api/studio/gallery.ts',
    from: '.bind(me.id).all()', to: '.bind(me.id).all().catch(() => ({ results: [] }))' },
  { g: '정직', name: 'CRM 집행 목록 조회를 다시 삼킨다',
    file: 'functions/api/crm/campaigns.ts',
    from: '.all()).results as any[]) || []', to: '.all().catch(() => ({ results: [] }))).results as any[]) || []' },
  { g: '정직', name: '플레이스 추적 목록 조회를 다시 삼킨다 (빈 목록이 캐시된다)',
    file: 'functions/api/naver-place/tracking.ts',
    from: "      return c.json({ success: false, error: '추적 목록을 불러오지 못했습니다.', keywords: [] }, 500)\n    }\n\n    return c.json({", to: '    }\n\n    return c.json({' },
  { g: '정직', name: '신청자 알림 설정을 못 읽어도 기본값을 보여 준다 (저장하면 켜 뒀던 설정이 덮인다)',
    file: 'functions/api/_leadalert.ts',
    from: "  const row: any = await db.prepare('SELECT value FROM settings WHERE key = ?').bind(KEY(String(userId))).first()\n  const raw = row ? row.value : null",
    to: '  const raw = await getSetting(db, KEY(String(userId)))' },
  { g: '정직', name: '관리자 대기 건수를 못 세면 0 으로 답한다 (승인 요청이 쌓여도 배지가 0)',
  ...
  { g: '크론', name: '프로그램 API 예외를 없애 curl 고객을 다시 자동 차단한다',
    file: 'functions/_middleware.ts',
    from: 'const badUa = BAD_UA_RE.test(ua) && !MACHINE_PATH_RE.test(path)', to: 'const badUa = BAD_UA_RE.test(ua)' },
  { g: '크론', name: '공격 도구 즉시 차단을 다시 못 돌게 만든다 (이름만 남는다)',
    file: 'functions/_middleware.ts',
    from: 'if (suspiciousPath || badUa || attackTool) {', to: 'if (suspiciousPath || badUa) {' },
