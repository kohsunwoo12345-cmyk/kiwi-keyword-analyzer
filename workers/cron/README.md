...
| 크론 | UTC | 한국시각 | 하는 일 |
|---|---|---|---|
| `* * * * *` | 1분마다 | 1분마다 | `/api/cron/video-schedules` — 때가 된 예약 영상 생성 |
| `5 0 * * *` | 00:05 | 09:05 | `/api/naver-place/update-all-tracking` — 플레이스 순위 추적(20건씩 전량) |

## 비용

- Workers 무료 플랜: **하루 10만 요청**. 1분마다 = 하루 1,440회 + 네이버 1회 → 한도의 1.4%.
  실행할 예약이 없으면 GET 한 번(대기 건수 확인)으로 끝나므로 사이트 부하도 미미하다.
...
1분 간격인 이유: 예약 시각(정시)에 최대한 붙여 실행하기 위해서다. 주기가 길면 크론이
한 번 밀렸을 때 그만큼 통째로 늦어진다. 인도(UTC+5:30)·네팔(UTC+5:45)처럼 30·45분
오프셋 지역도 자연히 커버된다.
...
**Logs**(또는 Cron Events 탭)에서 확인한다. `[cron] * * * * * 완료 — N단계` 로그가 남는다.
...
# 크론이 발화한 것처럼 실행
curl "http://127.0.0.1:8790/__scheduled?cron=*+*+*+*+*"
...
- 예약 영상은 Pages 쪽에서 한 번에 5건씩만 처리한다(과금 폭주 방지). 워커가 최대
  5회까지 이어서 두드리므로 한 번 실행에 최대 25건. 그 이상 밀리면 1분 뒤에 마저 돈다.
