...
## 부록 C. 단가 — 어디까지 확인됐고 어디부터 추정인가 (**A / C 혼재**)

공식 가격표 원문은 끝내 못 열었다. 개발 환경에서도 403 이고, 운영 워커로 읽는
`diag=prices` 도 (배포 시점 기준) 알리바바 항목이 아직 안 올라와 있었다.
다만 **서로 다른 두 출처가 같은 값을 가리키는** 구간이 생겼다.

### C-1. 확인된 것 (**A**)

| 모델 | 720P | 1080P | 근거 |
|---|---|---|---|
| `wan2.7-*` (t2v·i2v·r2v·videoedit) | **$0.086/초** | **$0.144/초** | 알리바바 공개 ¥0.6 / ¥1.0 · 별도 출처의 $0.086 / $0.144 — 환산이 서로 맞는다 |
| `wan2.6-t2v · i2v · r2v` | **$0.08/초** | **$0.12/초** | 재판매 사업자 두 곳이 같은 값 |

### C-2. 추정 (**C**) — 확인되면 내린다

wan2.5 이하, flash·turbo 계열, animate·vace, 모든 이미지 모델.
**1080p 값을 비싼 쪽으로 두고, 720p 는 그 0.6 배**로 계산한다.

### C-3. 여기서 실제로 고친 것 — 720p 를 26% 덜 받고 있었다

우리 기본 계산은 1080p 값에 **화소비**를 곱한다(720p = 0.444).
그런데 알리바바 표는 720P ¥0.6 / 1080P ¥1.0 = **0.6 배**다. 화소비가 아니다.

그대로 뒀으면 720p 를 원가의 **74%**(0.444 ÷ 0.6)만 받았다. 나머지 26% 는 우리가 문다.
영상은 720p 로 뽑는 일이 훨씬 많으니 이게 기본값에서 새는 구멍이 될 뻔했다.

그래서 루마·Veo 처럼 구간 값을 표에 적고 `wanUsd()` 가 그대로 쓴다. 실측:

```
Wan 2.7 t2v · 5초 · 배수 1(원가 그대로)
   720p  → ₩602    (5 × $0.086 × 1400)
  1080p  → ₩1,008  (5 × $0.144 × 1400)
  비율 0.597  ← 공개 표(0.6)와 일치
wanUsd() 를 빼면 비율이 0.444 로 떨어지고 720p 가 ₩448 이 된다 — 실제로 재현했다.
```

1080p 미만(480p·540p)은 **720p 값을 그대로 쓴다.** 더 싼 구간이 공개돼 있지 않은데
임의로 깎으면 그만큼 우리가 손해다.

---

## 부록 D. 단가 확정 — 공식 사이트는 회사 정책으로 막혀 있다 (**A / C 혼재**)

### D-1. 먼저 진단을 정정한다

앞에서 `alibabacloud.com`·`help.aliyun.com` 이 "403" 이라고 적었는데, 정확히는
**우리 회사 egress 정책에서 차단된 도메인**이다(`EGRESS_BLOCKED`). 사이트가 우리를
막은 게 아니라 우리 쪽에서 못 나가게 돼 있다. `evolink.ai`·`docs.apiyi.com`·
`aliyunbaike.com` 도 같다. 프록시 안내문이 "조직 정책 거부는 재시도하지 말고
보고하라" 고 하므로 **우회하지 않았다.**

### D-2. 그래서 어떻게 확인했나

공식 표를 그대로 옮겨 적은 곳을 여럿 읽고 **서로 어긋나지 않는 값만** 확정으로 올렸다.
독립된 두 저장소가 글자 하나 다르지 않은 같은 표를 냈다.

| 구간 | 720P | 1080P | 확인 |
|---|---|---|---|
| `wan2.7-*` · `wan2.6-t2v/i2v/r2v` · `wan2.5-*` 영상 | **¥0.6/초 = $0.086** | **¥1.0/초 = $0.144** | 출처 3곳 일치 + 국제판 공개가 환산 일치 |
| `wan2.2-t2v-plus` · `wan2.2-i2v-plus` | **¥0.4/초 = $0.057** | **¥0.6/초 = $0.086** | 출처 2곳 |
| `wan2.6-t2i` 이미지 | — | **¥0.2/장 = $0.029** | 출처 2곳 일치 |

**무료 체험 — 영상 50초 · 이미지 50장, 개통 후 90일. 실패는 과금되지 않는다**
(성공한 것만 센다). 즉 첫 시험 생성은 이 범위 안에서 돈이 안 나갈 수 있다.

### D-3. 값이 안 나온 것은 깎지 않는다

`flash` · `turbo` · `animate` · `vace` · `kf2v` · `2.1` 계열 · `wan2.7-image` ·
Qwen 이미지 전부. 이름만 보면 싸 보이지만 **추측해서 깎으면 원가보다 싸게 파는 쪽으로
틀린다.** 그래서 **같은 계열의 확정값을 그대로 상한으로 쓴다.**

- 싸게 잡아 틀리면 → 파는 만큼 손해, 되돌릴 수 없다
- 비싸게 잡아 틀리면 → 회원에게 더 받고, 확인되면 내리면 된다

관리자 화면은 확정(A)과 상한(C)을 갈라 보여 준다 — 상한에만 **[잠정]** 딱지가 붙는다.
전부 잠정이면 확정된 값까지 못 믿게 되고, 전부 확정처럼 보이면 상한을 믿고 팔게 된다.

### D-4. 실측으로 맞춘 것

```
Wan 2.7 t2v   720p 5초 → ₩602    (¥0.6/초 = $0.086)
              1080p 5초 → ₩1,008 (¥1.0/초 = $0.144)   비율 0.597 = 공개 표 0.6
Wan 2.2 Plus  720p 5초 → ₩399    (¥0.4/초 = $0.057)
Wan 2.6 t2i   1장     → ₩41     (¥0.2/장 = $0.029)
```
확정값을 하나 틀리게 바꿔 보면 검사가 그 자리를 짚는다(재현함).

### D-5. 아직 남은 단 하나

**위 값은 중국(百炼) 표기다.** 국제판(우리 키가 붙은 싱가포르)은 표가 따로 있을 수 있다.
다만 wan2.7 은 국제판 공개가 $0.086/$0.144 가 중국 표 환산과 정확히 맞았다 —
두 리전이 같은 값일 가능성이 높다는 뜻이지 확인은 아니다.
**최종 확인은 첫 청구서다.** 실제 금액이 나오면 관리자 → 모델 단가에서 덮어쓴다
(그 값이 언제나 코드 값을 이긴다).
